...
exec { 'enable and configure ufw for Nginx HTTP':
  command     => '/usr/sbin/ufw --force enable && /usr/sbin/ufw allow "Nginx HTTP"',
...
exec { 'update nginx configuration and restart':
  command  => "/bin/sed -i 's@try_files \\$uri \\$uri/ =404;@try_files \\$uri \\$uri/ /404.html;@' /etc/nginx/sites-available/default && /usr/sbin/service nginx restart",
  path     => ['/usr/bin', '/bin', '/usr/sbin'],
  provider => 'shell',
  require  => File['configuration file'],
}
